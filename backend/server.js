...
async function handleFirebaseLogin(req, res) {
  let body;
  try {
    body = await readBody(req, 102400);
  } catch (e) {
    return sendJson(res, 400, { error: "bad_request" });
  }
  try {
    const projectId = auth.firebaseCfg().projectId;
    const claims = await auth.verifyFirebaseIdToken(body.idToken, projectId);
    if (!claims.email || claims.email_verified === false) {
      return sendJson(res, 403, { error: "email_unverified" });
    }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "POST" && p === "/api/auth/firebase") {
